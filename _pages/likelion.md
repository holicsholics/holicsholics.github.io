...
tf.keras.layers.Flatten(input_shape=(28, 28)),  #  2차원 배열(28 x 28 픽셀)의 이미지를 1차원으로 flatten 
tf.keras.layers.Dense(128, activation='relu'),  # 128개의 노드(뉴런)을 가지고 연결
...
```  
  - 컴파일 `model.compile()`
```python
model.compile(optimizer='rmsprop',  # 옵티마이저: 손실함수의 값을 가능한 낮추는 매개변수 탐색
                loss='categorical_crossentropy',  # 손실함수 설정
                metrics=['accuracy'])  # 각 훈련 epoch에 대한 훈련 및 검증 정확성을 보려면 metrics 인수를 전달
```
  - 요약 `model.summary()`
```python
model.summary()
...
# 이 콜백은 3회 연속 손실 개선이 없을 때 훈련을 중단합니다.
history = model.fit(np.arange(100).reshape(5, 20), np.zeros(5),
                    epochs=10, batch_size=1, callbacks=[callback],verbose=0)
len(history.history['loss'])  # 3회 연속 손실 개선이 없을 때 훈련을 중단된 지점
>> 4
```  
  - 예측 `model.predict()`
  - 평가 `model.evaluate()`
